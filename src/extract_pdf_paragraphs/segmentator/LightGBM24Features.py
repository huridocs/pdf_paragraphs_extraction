    ...
    def get_predicted_segments(self, pdfalto_xml, page_tags: List[PdfTag]) -> List[PdfSegment]:
        ...
        for i in range(context_size):
            page_tags.insert(
                ...
                PdfTag(
                    ...
                    "pad_tag",
                    "PADFONT",
                    PdfFont("PADSTYLE", False, False, 0),
                    -i - 1,
                    -i - 1,
                    Rectangle(0, 0, 0, 0),
                ),
        ...
        for tag_index, tag in enumerate(page_tags):
            ...
            for i in range(2 * context_size + 1):
                new_data_row.extend(
                    pdfalto_xml.get_features_for_given_tags(
                        page_tags[tag_index + i],
                        page_tags[tag_index + i + 1],
                        page_tags,
                    )
                )
        ...
        for segment in segments_by_tags:
            ...
            segments.append(
                PdfSegment(
                    page_tags[0].page_number,
                    segment_ids,
                    Rectangle.from_pdftags(segment),
                    text_content,
                )
            )
